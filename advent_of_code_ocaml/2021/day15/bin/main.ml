...
module Cell = struct
  ...
  let pp ?(verbosity = 0) ppf { i; j; risk; distance; visited } =
    if verbosity = 0 then Format.fprintf ppf "%d" risk
    else if verbosity = 1 then Format.fprintf ppf "%d(%d)" risk distance
    else
...
let () =
  ...
  let algo = try Sys.argv.(3) with _ -> "1" in
